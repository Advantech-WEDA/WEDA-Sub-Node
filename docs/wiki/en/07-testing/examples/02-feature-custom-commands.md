---
doc-id:    E2E-EXD-FEATURE_CUSTOM_COMMANDS
title:     "feature-custom-commands — End-to-End Deployment Test Report"
...
# feature-custom-commands — End-to-End Deployment Test Report
...
| | |
|---|---|
| **Example** | [`examples/feature-custom-commands`](../../../../../examples/feature-custom-commands/) |
| **Compose service** | `command-handler` |
| **Image** | `harbor.arfa.wise-paas.com/edge-coa/feature-custom-commands:latest` |
| **Transport** | Modbus TCP (embedded simulator) |
| **External dependency** | None |
| **SubNode name used** | `CmdHandlerDemo-<date>` |
| **Registered deviceId** | — |
...
- `Demo command 'sensor.read' succeeded` — value 25.464, unit celsius
- `Demo command 'tag.set' succeeded` — 3 tags applied
- `Demo command 'props.get' succeeded` — device properties returned

## 3. Defects this example exposed

- D-01 WedaNode URL `172.22.160.192:4224`
...
Had neither a Dockerfile nor a compose file before this work. Its custom commands are the SubNode function under test, and all three executed on the device.

## 5. Re-running this test

```bash
# 1. Build and publish (repository root is the build context — the script enforces it)
PLATFORMS=linux/arm64 ./scripts/build-push-image.sh feature-custom-commands
