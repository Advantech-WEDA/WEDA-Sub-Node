---
doc-id:    E2E-EXD-VISION_OBJECT_DETECTION
title:     "vision-object-detection — End-to-End Deployment Test Report"
...
# vision-object-detection — End-to-End Deployment Test Report
...
| | |
|---|---|
| **Example** | [`examples/vision-object-detection`](../../../../../examples/vision-object-detection/) |
| **Compose service** | `vision-subnode` |
| **Image** | `harbor.arfa.wise-paas.com/edge-coa/vision-object-detection:latest` |
| **Transport** | MQTT (detection messages from the YOLO CV stack) |
| **External dependency** | MQTT broker — **bundled**; the CV stack publishes in production |
| **SubNode name used** | `VisionDemo-<date>` |
| **Registered deviceId** | 355914456695308288 |
...
- `Subscribed to vision topics: advantech/+/vision/detections, .../status, .../meta`
- `Message broker subscription established`
- `Health report sent successfully`

## 3. Defects this example exposed

- D-01 WedaNode port `4222` (should be 4224)
- D-03 literal `REPLACE_WITH_NATS_USER`
...
Carried two independent credential defects. The bundled broker stands in for the CV stack, so the SubNode subscribes and registers but sees no detections until a real publisher is attached.

## 5. Re-running this test

```bash
# 1. Build and publish (repository root is the build context — the script enforces it)
PLATFORMS=linux/arm64 ./scripts/build-push-image.sh vision-object-detection
