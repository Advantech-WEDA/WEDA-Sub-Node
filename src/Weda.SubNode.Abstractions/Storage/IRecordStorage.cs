...
{
    Task WriteAsync(string sensorId, RecordingDataPoint dataPoint, CancellationToken cancellationToken = default);

    Task WriteBatchAsync(string sensorId, IEnumerable<RecordingDataPoint> dataPoints, CancellationToken cancellationToken = default);
