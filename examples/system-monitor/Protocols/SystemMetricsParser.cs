...
public class SystemMetricsParser : IRequestResponseProtocolParser
{
    ...
    private List<TelemetryMeasure> ConvertToTelemetryMeasures(List<Sensor> sensors, SystemMetricsRawData rawData)
    {
        ...
        foreach (var sensor in sensors)
        {
            ...
            {
                _logger.LogWarning("Sensor {Name} missing MetricType or MetricName parameter", sensor.Name);
                continue;
            }
    ...
    private object? GetMetricValue(SystemMetricsRawData rawData, string metricType, string metricName, Sensor sensor)
    {
        ...
        {
            "cpu" => GetCpuMetric(rawData.Cpu, metricName),
            "memory" => GetMemoryMetric(rawData.Ram, metricName),
            "disk" => GetDiskMetric(rawData.Disks, metricName, sensor),
            "network" => GetNetworkMetric(rawData.Networks, metricName, sensor),
            _ => null
        };
    }
