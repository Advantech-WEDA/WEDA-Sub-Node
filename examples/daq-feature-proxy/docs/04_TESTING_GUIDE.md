...
| `daq-data-collector` | Publishes PHM feature telemetry | Running and registered with WEDA Node |
...
**Test Objective**: Verify end-to-end capability query against a live NATS broker with `daq-data-collector` registered.
...
| `daq-data-collector` not registered | Capability query times out after 3 retries | Start and wait for DAQ to register |
