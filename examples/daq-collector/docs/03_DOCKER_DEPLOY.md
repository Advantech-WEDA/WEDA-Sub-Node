...
- Use `-f examples/daq-collector/Dockerfile` to specify Dockerfile path, build context is root directory (`.`)
...
```bash
# Using scp
cd /home/advantech/vincent/edge_subnode/examples/daq-collector
...
```bash
cd /home/advantech/vincent/edge_subnode
docker buildx build --platform linux/arm64 -f examples/daq-collector/Dockerfile -t daq-data-collector:latest --load .
```
