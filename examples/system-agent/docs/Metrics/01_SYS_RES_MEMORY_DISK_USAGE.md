...
## 4. Value validators driven by the DTDL `comment` field

```csharp
public sealed class MemoryDiskSystemValueValidator
{
    ...
    private static readonly Dictionary<string, Func<object?, bool>> Predicates = new()
    {
        ...
        [SystemResourceDtmis.Telemetries.MemoryTotal]      = v => v is long l && l > 0,
        [SystemResourceDtmis.Telemetries.MemoryAvailable]  = v => v is long l && l >= 0,
        [SystemResourceDtmis.Telemetries.MemoryUsed]       = v => v is long l && l >= 0,
        [SystemResourceDtmis.Telemetries.MemoryFree]       = v => v is long l && l >= 0,
        [SystemResourceDtmis.Telemetries.MemoryCached]     = v => v is long l && l >= 0,
        [SystemResourceDtmis.Telemetries.MemoryBuffers]    = v => v is long l && l >= 0,
        [SystemResourceDtmis.Telemetries.MemorySwapTotal]  = v => v is long l && l >= 0,
        [SystemResourceDtmis.Telemetries.MemorySwapFree]   = v => v is long l && l >= 0,

        // disk
        [SystemResourceDtmis.Telemetries.DiskTotal]           = v => v is long l && l >= 0,
        [SystemResourceDtmis.Telemetries.DiskAvailable]       = v => v is long l && l >= 0,
        [SystemResourceDtmis.Telemetries.DiskFree]            = v => v is long l && l >= 0,
        [SystemResourceDtmis.Telemetries.DiskUsed]            = v => v is long l && l >= 0,
        [SystemResourceDtmis.Telemetries.DiskUsagePercent]    = v =>
            v is double d && d >= 0 && d <= 100 && !double.IsNaN(d) && !double.IsInfinity(d),
        [SystemResourceDtmis.Telemetries.DiskReadsCompleted]  = v => v is long l && l >= 0,
        [SystemResourceDtmis.Telemetries.DiskWritesCompleted] = v => v is long l && l >= 0,
        [SystemResourceDtmis.Telemetries.DiskReadBytes]       = v => v is long l && l >= 0,
        [SystemResourceDtmis.Telemetries.DiskWrittenBytes]    = v => v is long l && l >= 0,

        // system
        [SystemResourceDtmis.Telemetries.SystemTime]            = v => v is long l && l >= EpochMin && l < EpochMax,
        [SystemResourceDtmis.Telemetries.SystemTimexOffset]     = v =>
            v is double d && !double.IsNaN(d) && !double.IsInfinity(d),
        [SystemResourceDtmis.Telemetries.SystemBootTime]        = v => v is long l && l >= EpochMin && l < EpochMax,
        [SystemResourceDtmis.Telemetries.SystemFilefdAllocated] = v => v is long l && l >= 0,
        [SystemResourceDtmis.Telemetries.SystemFilefdMaximum]   = v => v is long l && l > 0,
        [SystemResourceDtmis.Telemetries.SystemProcsRunning]    = v => v is int i && i >= 0,
        [SystemResourceDtmis.Telemetries.SystemProcsBlocked]    = v => v is int i && i >= 0,
        [SystemResourceDtmis.Telemetries.SystemIntrTotal]       = v => v is long l && l >= 0,
    };
...
Cross-field invariants (e.g. `MemoryUsed + MemoryAvailable ≈ MemoryTotal`, `MemoryFree ≤ MemoryTotal`, `SystemFilefdAllocated ≤ SystemFilefdMaximum`) live outside this predicate map — implement them after a coherent batch of telemetry arrives.

---

## 5. Monotonic-counter checks (Disk I/O + `system.intr_total`)

Apply the `MonotonicCounterValidator` pattern from `01_SYS_RES_CPU_NETWORK_USAGE.md` §6. Apply to:
...
public sealed class MemoryDiskSystemPublisher
{
    private readonly ICommunication _communication;
    private readonly MemoryDiskSystemValueValidator _validator;
    private readonly ILogger<MemoryDiskSystemPublisher> _logger;

    public MemoryDiskSystemPublisher(
        ICommunication communication,
        MemoryDiskSystemValueValidator validator,
        ILogger<MemoryDiskSystemPublisher> logger)
    {
    ...
    public Task PublishMemoryUsedAsync(long bytes, CancellationToken ct = default)
        => PublishAsync(SystemResourceDtmis.Telemetries.MemoryUsed, bytes, ct);

    public Task PublishDiskUsagePercentAsync(string mountPoint, double pct, CancellationToken ct = default)
        => PublishAsync(SystemResourceDtmis.Telemetries.DiskUsagePercent,
                        Math.Round(pct, 2), ct,
                        discriminator: mountPoint);

    public Task PublishSystemTimeAsync(long unixEpoch, CancellationToken ct = default)
        => PublishAsync(SystemResourceDtmis.Telemetries.SystemTime, unixEpoch, ct);
        ...
        string dtmi, object value, CancellationToken ct, string? discriminator = null)
    {
        ...
        {
            _logger.LogWarning("Dropping {Dtmi}={Value} (failed validation)", dtmi, value);
            return;
        }
