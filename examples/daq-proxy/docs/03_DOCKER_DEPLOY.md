...
- Use `-f examples/daq-proxy/Dockerfile` to specify the Dockerfile path; build context is root (`.`)
...
```bash
cd /home/advantech/vincent/edge_subnode/examples/daq-proxy
...
> **Note**: Unlike `daq-collector`, this service requires **no** `privileged` mode, no `pid: host`, no device mappings, and no system library mounts. It is a pure-software container.
...
```bash
cd /home/advantech/vincent/edge_subnode
docker buildx build --platform linux/arm64 -f examples/daq-proxy/Dockerfile -t daq-feature-proxy:latest --load .
```
...
3. Verify `daq-collector` is running and publishing telemetry
